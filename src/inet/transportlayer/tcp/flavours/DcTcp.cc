...
{
    EV_INFO << "\nDCTCPInfo ___________________________________________" << endl;
    EV_INFO << "\nDCTCPInfo - Received Data Ack" << endl;
    ...
    if (state->dupacks >= state->dupthresh) {
        ...
        if (state && state->ect) {
            ...
            // RFC 8257 3.3.3
            if (state->gotEce) {
                EV_INFO << "\nDCTCPInfo - Marked ACK arrived - increasing bytes marked" << endl;
                state->dctcp_bytesMarked += bytes_acked;
                EV_INFO << "\nDCTCPInfo - Total bytes marked: " << state->dctcp_bytesMarked << endl;
                conn->emit(markingProbSignal, 1);
            }
            else {
                EV_INFO << "\nDCTCPInfo - Non-marked ACK arrived" << endl;
            ...
            if (state->snd_una > state->dctcp_windEnd) {
                ...
                // RFC 8257 3.3.5
                double ratio;
                EV_INFO << "\nDCTCPInfo - End of current window - calculating ratio" << endl;
                ratio = ((double)state->dctcp_bytesMarked / state->dctcp_bytesAcked);
                EV_INFO << "\nDCTCPInfo - state->dctcp_bytesMarked = "<< state->dctcp_bytesMarked << endl;
                EV_INFO << "\nDCTCPInfo - state->dctcp_bytesAcked = "<< state->dctcp_bytesAcked << endl;
                EV_INFO << "\nDCTCPInfo - state->dctcp_bytesMarked / state->dctcp_bytesAcked = "<< ratio << endl;
                conn->emit(loadSignal, ratio);

                // RFC 8257 3.3.6
                // DCTCP.Alpha = DCTCP.Alpha * (1 - g) + g * M
                EV_INFO << "\nDCTCPInfo - Previous Alpha Value: "<< state->dctcp_alpha << endl;
                state->dctcp_alpha = state->dctcp_alpha * (1 - state->dctcp_gamma) + state->dctcp_gamma * ratio;
                EV_INFO << "\nDCTCPInfo - New Alpha Value: "<< state->dctcp_alpha << endl;
            ...
            if ((state->dctcp_bytesMarked && !state->sndCwr)) {
                ...
                // RFC 8257 3.3.9
                EV_INFO << "\nDCTCPInfo - Updating CWND. Previous value: "<< state->snd_cwnd << endl;
                state->snd_cwnd = state->snd_cwnd * (1 - state->dctcp_alpha / 2);
                EV_INFO << "\nDCTCPInfo - New CWND: "<< state->snd_cwnd << endl;
                conn->emit(cwndSignal, state->snd_cwnd);
